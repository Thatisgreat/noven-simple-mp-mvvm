...
var uid = 1;
var targetStack = []; //保存所有的target
var arrayHandler = handleArray();

//核心类

var Noven = function () {
	function Noven($options) {
		_classCallCheck(this, Noven);
  ...
  */
	...
	_createClass(Noven, [{
		key: 'walk',
		value: function walk(obj, isRoot) {
			var _this = this;
			...
			Object.entries(obj).forEach(function (_ref) {
				...
				_this.defineReactive(obj, key, value, isRoot);
			});
		}
   ...
   */
		...
		key: 'defineComputed',
		value: function defineComputed(key, func) {
			var nvm = this;
...
Noven.prototype.initComputed = function () {
	...
	Object.entries(this.$options.computed).forEach(function (_ref3) {
		...
		//如果是对象，则取这个对象的get
		var getter = typeof value === 'function' ? value : value.get;
		getter = getter || noop;

		//每一个computed都是一个watcher
		watchers[key] = new Watcher(_this2, getter, noop);

		//将每一个computed的key，代理到nvm身上，可以通过this.xxx来访问这个computed
		_this2.defineComputed(key, getter);
	});
};
//watch对象
Noven.prototype.initWatch = function () {
	...
	Object.entries(this.$options.watch).forEach(function (_ref5) {
		var _ref6 = _slicedToArray(_ref5, 2),
		    key = _ref6[0],
		    value = _ref6[1];

		//每一个watch都是一个watcher
		new Watcher(_this3, key, value);
	});
};
...
Noven.prototype.initMethods = function () {
	...
	Object.entries(this.$options.methods).forEach(function (_ref7) {
		var _ref8 = _slicedToArray(_ref7, 2),
		    key = _ref8[0],
		    value = _ref8[1];

		_this4[key] = value.bind.apply(value, [_this4].concat(Array.prototype.slice.call(_arguments)));
	});
};
...
Dep.prototype.addSubs = function (watcher) {
	...
	if (!this.subs.find(function (sub) {
		return sub.id === id;
